...
function CheckoutForm({ track, onSuccess, onCancel }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "#1a1a1a", padding: 16, borderRadius: 8, border: `1px solid ${chrome}` }}>
        <PaymentElement />
      </div>
      {error && <div style={{ color: neonRed, fontSize: 13 }}>{error}</div>}
      <button style={{ ...s.btnNeon, opacity: loading ? 0.7 : 1 }} onClick={handleSubmit} disabled={loading}>
        {loading ? "Behandlar..." : `Betala ${CONFIG.PRICE_PER_SONG} kr`}
      </button>
      <button style={s.btnGhost} onClick={onCancel}>Avbryt</button>
    </div>
  );
}
      ...
      <div style={s.app}>
        ...
        {guestQueueCount > 0 && (
          <div style={s.ticketSection}>
            {guestQueue.map((t, i) => (
              <div key={t.track_id} style={s.ticket}>
                <div style={s.ticketNum}>{i + 1}</div>
                <div style={s.ticketInfo}>
                  <div style={s.ticketName}>{t.track_name}</div>
                  <div style={s.ticketArtist}>{t.artist_name}</div>
                </div>
                <div style={s.ticketNote}>I KÖN</div>
              </div>
            ))}
          </div>
        )}
        ...
        {paymentStep === "full" && (
          <div style={s.overlay} onClick={handleClose}>
            <div style={s.modal} onClick={e => e.stopPropagation()}>
              ...
              <div style={s.modalDivider} />
              <p style={s.modalText}>Just nu är det {CONFIG.MAX_QUEUE_SIZE} låtar i kön. Vänta lite och försök igen!</p>
              <button style={s.btnNeon} onClick={handleClose}>STÄNG</button>
            </div>
          </div>
        )}
